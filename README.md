# Windows DFIR Lab 66 — AMSI-Related Tampering Investigation
...
- Understand AMSI from a defensive DFIR perspective.
- Establish a baseline for `amsi.dll`.
- Record the AMSI DLL hash, version, timestamps, and metadata.
- Inspect AMSI-related registry configuration.
- Review PowerShell Script Block Logging telemetry.
- Review Sysmon Event ID 1 and Event ID 13.
- Correlate endpoint telemetry with Wazuh.
- Distinguish relevant security events from unrelated registry activity.
- Investigate Windows Event Log availability when telemetry collection fails.
- Build an evidence-based investigation timeline.
- Avoid treating a single suspicious-looking event as proof of AMSI tampering.
...
Evidence/
├── baseline.txt
├── execution-policy-baseline.txt
├── amsi-registry.txt
└── sysmon-registry-events.txt
```

Additional evidence can be added as the investigation progresses.
...
- Establish a baseline before interpreting changes.
- File hashes are useful for integrity validation.
- Registry Event ID 13 requires contextual analysis.
- Process creation telemetry helps identify the actor behind a change.
- PowerShell Event ID 4104 can provide script-level context.
- Wazuh can centralize Sysmon telemetry for investigation.
- Event Log availability is itself an important forensic consideration.
- A missing or suspicious artifact does not automatically prove malicious activity.
- Correlation across multiple evidence sources produces stronger conclusions.
...
AMSI Tampering: Not conclusively established
AMSI Baseline: Established
PowerShell Telemetry: Reviewed
Sysmon Telemetry: Reviewed
Wazuh Correlation: Performed
Event Log Issue: Identified
