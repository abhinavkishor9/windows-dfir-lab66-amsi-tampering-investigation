# Timeline — Lab 66 AMSI-Related Tampering Investigation

## Timeline Overview

This timeline records the major investigation activities performed during Lab 66.

The timeline combines the actual timestamps visible in the collected screenshots with the sequence of investigative actions. Where an exact timestamp was not captured, the event is marked as an approximate investigation phase rather than assigning an invented time.
...
Windows Event Log Issue:
Identified

Confirmed AMSI Tampering:
No
```
...
The investigation established a baseline for AMSI-related components and demonstrated visibility through PowerShell, Sysmon, and Wazuh.

Registry modification activity was observed, but the available Wazuh example targeted `WpnUserService` rather than AMSI. Therefore, the evidence does not support a definitive conclusion that AMSI was tampered with.

The investigation also uncovered a separate endpoint telemetry problem in which the Windows Event Log service became stuck in `START_PENDING`, causing both `Get-WinEvent` and `wevtutil` to return `The interface is unknown`.

This troubleshooting event is itself an important part of the forensic record because it affected the availability and reliability of Windows event telemetry.
